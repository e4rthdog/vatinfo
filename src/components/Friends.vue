...
const loadCID = async () => {
  $q.loading.show({
    message: "Loading your favorite Pilots/Controllers ...",
...
const saveCID = async () => {
  $q.loading.show({
    message: "Saving your favorite Pilots/Controllers ...",
    spinner: QSpinnerHourglass,
  });
    ...
    <q-slide-transition>
      <q-card-section v-show="panelVisible">
        <q-markup-table
          ...
          flat
          wrap-cells
          class="text-center e4-panel-table-lg"
        >
          <thead>
            <tr class="bg-grey-1">
              ...
              <tr v-if="f.callsign">
                <td class="text-left">
                  <q-badge class="q-pa-xs" color="green">
                    <q-icon
                      :name="appConfig.status[f.clienttype]"
                      class="q-mr-xs"
                    />{{ f.clienttype }}
                  </q-badge>
                </td>
    ...
    <q-card style="width: 300px">
      ...
      <q-card-section class="q-pa-xs">
        <div class="col-12 col-lg-7 q-ma-xs">
          <q-input
            id="txtCID"
            @keypress.enter="formAddCID()"
            color="positive"
            v-model.trim="txtCID"
            label="CID, Callsign (Pilot), Callsign* (ATC)"
            dense
          >
      ...
      >
        <div
          v-for="(f, index) in cfgStore.arrCIDS"
          :key="index"
          class="column inline"
        >
